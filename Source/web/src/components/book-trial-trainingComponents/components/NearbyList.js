...
import { PARENT_API } from 'src/requests/ApiConfig';
import { sendGet, attachTokenToHeader } from 'src/services/httpMethodPA';
...
export default (props) => {
    ...
    //! UseEffect
    useEffect(() => {
        attachTokenToHeader(props?.bookingFull?.access_token);
    }, []);
    ...
    const getAvalableCourse = async () => {
        await sendGet(
            `${PARENT_API}get-available-courses${Utils.convertToQuery({
                child_id: props?.bookingFull?.child_id,
                ...
                {listCourse.length > 0 ? (
                    ...
                    <div>
                        <p style={{ color: '#ff7100', fontSize: '30px' }}>
                            Please select another nearby academy
                        </p>
                    </div>
                )}
            </div>
