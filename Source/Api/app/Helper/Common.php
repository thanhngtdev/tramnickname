...
//        return time().uniqid(true);
    ...
    public static function calPriceAssignee($quality,$quantity,$priceUnit){
        $lstQuality = \App\Http\DAL\DAL_Config::getConfigByLocale(7);
        $lastQuality = end($lstQuality);
        if ($quality == $lastQuality['id'])
            $assigneePrice = $priceUnit * .88;
        elseif ($quantity < 100) $assigneePrice = $priceUnit * .75;
        else $assigneePrice = $priceUnit * .83;
        return 5 * 100 * round($assigneePrice / 100 / 5);
    }
}